...
describe('Should authenticate usinig Puppeteer through a One Login Auth that has seperate steps for entering Username and Password', () => {
  context('Authentication Example', () => {
    ...
    before(() => {
      ...
      Cypress.env(
        'authUrl',
        Cypress.env('domains')[environment]['defaultApplication']
      );
      ...
      cy.then(() => {
        expect(Cypress.env('TokenSet')).to.equal(
          true,
          'If this passed, authentication succeeded and passed localStorage to Cypress.'
        );
      });
    });
